...
export default defineConfig({
  ...
  esbuildOptions(options) {
    ...
    // Add a plugin to handle require calls
    options.plugins.push({
      name: 'polyfill-require',
      setup(build) {
        build.onResolve({ filter: /^(crypto|node:crypto)$/ }, () => {
          return { path: require.resolve('./polyfills/crypto.js'), namespace: 'polyfill' };
        });
  ...
  banner: {
    js: `
      ...
      (function() {
        ...
        // This must run immediately, not conditionally
        (function() {
          if (typeof globalObj.Buffer === 'undefined') {
            ...
            // Set Buffer on all global objects to ensure it's available everywhere
            globalObj.Buffer = BufferImpl;
            if (typeof window !== 'undefined') {
              window.Buffer = BufferImpl;
              // Also make it available via global
              if (typeof global !== 'undefined') global.Buffer = BufferImpl;
            }
            // Export Buffer so it's available immediately
            if (typeof exports !== 'undefined') exports.Buffer = BufferImpl;
          }
        })();
        // Ensure Buffer is available on window if we're in a browser
        if (typeof window !== 'undefined' && typeof window.Buffer === 'undefined' && typeof globalObj.Buffer !== 'undefined') {
          window.Buffer = globalObj.Buffer;
        }
